const STORAGE_KEY = 'dddd-alpah-extension::state';
...
async function handleControl(action) {
  ...
  try {
    ...
    console.error('[dddd-alpah-extension] Control request failed', error);
  } finally {
...
async function handleSpreadChange() {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.error('[dddd-alpah-extension] Failed to persist price offset', error);
...
async function handlePointsFactorChange() {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.error('[dddd-alpah-extension] Failed to persist points factor', error);
...
async function handlePointsTargetChange() {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.error('[dddd-alpah-extension] Failed to persist points target', error);
...
async function refreshActiveTabContext() {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.error('[dddd-alpah-extension] Unable to inspect active tab URL', error);
  }
}
